...
api_url = "https://apts.jp/api/properties.json?rent_range[max]=220000&bedroom_range[min]=0&bedroom_range[max]=2&rent_range[min]=&size_min=35"
puts api_url
...
while data['page']['current_page'].to_i != data['page']['total_pages'].to_i + 1
    ...
    api_url = "https://apts.jp/api/properties.json?rent_range[max]=220000&bedroom_range[min]=0&bedroom_range[max]=2&rent_range[min]=&size+min=35&page=" + (data['page']['current_page'] + 1).to_s
    response = openthis(api_url)
    data = JSON.parse(response)
end
